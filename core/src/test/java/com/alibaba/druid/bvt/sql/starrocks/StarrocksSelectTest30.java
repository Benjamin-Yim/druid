...
public class StarrocksSelectTest30 extends TestCase {
    public void test_select() throws Exception {
        // 1095288847322
        String sql = "select a/100.0 from dual where 1=1;";//
        assertEquals("SELECT a / 100.0\n" +
                "FROM dual\n" +
                "WHERE 1 = 1;", SQLUtils.formatStarrocks(sql));

        assertEquals("select a / 100.0\n" +
                "from dual\n" +
                "where 1 = 1;", SQLUtils.formatStarrocks(sql, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
