...
public class StarrocksSelectTest22 extends TestCase {
    public void test_select() throws Exception {
        ...
        assertEquals("SELECT bucket_id, sum(pv) AS pv, sum(clk) AS clk\n" +
                ...
                "\t\tOR CLK.clk > 0\n" +
                ") A\n" +
                "GROUP BY bucket_id;", SQLUtils.formatStarrocks(sql));
        ...
        assertEquals("select bucket_id, sum(pv) as pv, sum(clk) as clk\n" +
                ...
                "\t\tor CLK.clk > 0\n" +
                ") A\n" +
                "group by bucket_id;", SQLUtils.formatStarrocks(sql, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
