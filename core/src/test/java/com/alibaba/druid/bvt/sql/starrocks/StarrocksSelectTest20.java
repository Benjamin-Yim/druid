...
public class StarrocksSelectTest20 extends TestCase {
    public void test_select() throws Exception {
        // 1095288847322
        String sql = "select * from a DISTRIBUTE BY b,c";//
        Assert.assertEquals("SELECT *\n" +
                "FROM a\n" +
                "DISTRIBUTE BY b, c", SQLUtils.formatStarrocks(sql));
        Assert.assertEquals("select *\n" +
                "from a\n" +
                "distribute by b, c", SQLUtils.formatStarrocks(sql, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
